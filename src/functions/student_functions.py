...
def update_student_metadata(student_name: str, updates: StudentMetadataV2Update) -> Optional[StudentMetadataV2]:
    ...
    normalized_name = normalize_student_name(student_name)
    existing = get_student_metadata(normalized_name)
    if not existing:
        return None
...
def update_student_balance(student_name: str, new_balance: float) -> Optional[StudentV2]:
    """Update student balance in StudentsV2."""
    normalized_name = normalize_student_name(student_name)
    ...
    return get_student(normalized_name)
