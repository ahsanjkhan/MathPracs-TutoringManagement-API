...
def get_tutor_by_name(name: str) -> Optional[TutorV2]:
    ...
    for tutor in tutors:
        first_word = tutor.display_name.lower().split()[0] if tutor.display_name else ""
...
def create_tutor(display_name: str, calendar_id: str, access_role: str) -> TutorV2:
    ...
    tutor = TutorV2(display_name=display_name, calendar_id=calendar_id, access_role=access_role)
    dynamodb.put_item(settings.tutors_table, tutor.to_dynamodb())
    meta = TutorMetadataV2(tutor_id=tutor.tutor_id, display_name=display_name)
...
def update_tutor(tutor_id: str, updates: TutorV2Update) -> Optional[TutorV2]:
    """Update operational tutor fields in TutorsV2 (display_name, status)."""
    existing = get_tutor(tutor_id)
    if not existing:
        return None
